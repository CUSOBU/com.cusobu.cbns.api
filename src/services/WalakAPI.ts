...
const updateStatusNotification = async (req: Request, res: Response, next: NextFunction) => {
    ...
    switch (req.body.status) {
        case 'Pending': return res.status(404).json({ message: 'Invalid status Pending' }); break;
        case 'Delivery': {
        ...
        case 'Cancel': {
            if (remittance.status !== 'Pending' && remittance.status !== 'Delivery') {
        ...
        default: return res.status(400).json({ message: 'Invalid status' });
    }
